...
Decimals in the RDP, unlike the CPU are not represented in IEEE 754, instead they are represented using [fixed point arithmetic](https://en.wikipedia.org/wiki/Fixed-point_arithmetic). Later in this post you will see numbers such as `10.2`. This means that the variable has 10 bits of integer part and 2 bits of decimal part.
...
| ![peach_dedither.gif]({{ site.baseurl }}/assets/images/peach_dedither.gif) |
|:--:|
| *Final product comparison, 16 bpp vs 32 bpp* |

You might be thinking "why are these things even configurable, why would I want the dithering errors on my image, when it looks so much better without them?".
Which is a good question, but the RDP aims to be very configurable and leaves it up to the developer whether they want the dedithering filter or they think the dithering artifacts add some value to their scene.
...
| ![peach_dither_modes.gif]({{ site.baseurl }}/assets/images/peach_dither_modes.gif)
|:--:|
| *The four different dithering modes, dedithering disabled* |

Square and Bayer are algorithms. Square is described in the N64 programming manual to be the best when the dedithering filter is enabled, while Bayer
is described to be the best when it's disabled. Noise simply adds random noise. This is commonly used as an effect (think TV static) rather than for
dithering, but can be used for dithering as well.

There's another type of dithering, that's called Gamma Dithering. You enable this from the Video Interface and only when Gamma Correction is also enabled.
